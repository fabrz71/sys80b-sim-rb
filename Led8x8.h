/* *** Gottileb System 80/B SIM PRB ***
* (SIMulation Pinball Replacement control Board)
* software for Teensy 3.2 board developed with Arduino IDE
* under GNU General Public Livence v3
* by Fabrizio Volpi (fabvolpi@gmail.com)
* ---
* ON-board 8x8 LED array programmer interface
* ---
* Functions for on-board LED array.
* (Static Class)
*/

#ifndef _Led8x8_h_
#define _Led8x8_h_

enum lg_mode_enum { LG_OFF, LG_SWITCHES, LG_LIGHTS };

// reversed order bits byte (LSB->MSB)
PROGMEM byte revByte[] = {
	 };

class Led8x8 {

protected:
	static SPISettings _spis;
	static int _ss_pin;

public:
	static const byte CMD_DECODE = 0x9;
	static const byte CMD_INTNSTY = 0xa;
	static const byte CMD_DIGITS = 0xb;
	static const byte CMD_SHUTDWN = 0xc;

	static bool enabled;

protected:
	static void _writeCmd(byte adr, byte data);	

public:
	static void init(SPISettings ss, int pin);
	static void setLedRowReverse(byte row, byte data);
	static void setLedRow(byte row, byte data);
	static void clearLGgrid();
	static void shutDownMode(bool b);
	static void setLGintens(byte i);
};

#endif

