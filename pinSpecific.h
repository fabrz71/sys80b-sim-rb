// Game specific structure & rules

#include "pinGeneric.h"
...
void initPinball() {
  ...
  timers->add(dropt_tmr);
  timers->add(lshow_tmr);
}
...
void onBreakTarget(byte n) {
  if (_3BALLS) addScore(3000); else addScore(1000);
  addBonus(1000);
  if (break_ls.getLMode(n) == BLINK_L) {
    advanceJail();
    addScore(10000);
  }
  else jail_ls.pulse(n, 100);
